<script lang="ts">
	...
	const spacingTop = connectTop ? '' : 'pt-12';
	const spacingBottom = connectBottom ? '' : 'pb-12';
	...
	function parseMarkdown(text: string): string {
		...
		// Headers
		html = html.replace(/^### (.*$)/gim, '<h3 class="mb-4 mt-8 font-serif text-2xl font-bold text-primary-900">$1</h3>');
		html = html.replace(/^## (.*$)/gim, '<h2 class="mb-6 mt-10 font-serif text-3xl font-bold text-primary-900">$1</h2>');
		html = html.replace(/^# (.*$)/gim, '<h1 class="mb-6 mt-10 font-serif text-4xl font-bold text-primary-900">$1</h1>');
		
		// Bold
		html = html.replace(/\*\*(.*?)\*\*/g, '<strong class="font-semibold text-primary-900">$1</strong>');
		
		// Italic
		html = html.replace(/\*(.*?)\*/g, '<em class="italic">$1</em>');
		
		// Links
		html = html.replace(/\[(.*?)\]\((.*?)\)/g, '<a href="$2" class="text-accent underline hover:text-primary-900">$1</a>');
		...
		<div class="rounded-xl bg-white p-8 shadow-lg md:p-12">
			...
			{:else}
				<div class="rounded-lg border-2 border-dashed border-primary-300 bg-primary-50 p-8 text-center">
					...
					<p class="mt-2 text-xs text-primary-600">Supporte: # Titres, **gras**, *italique*, [liens](url), listes</p>
				</div>
			{/if}
		</div>
	</div>
