...
// Add fetchModel function from ingest-builder-content.mjs
async function fetchModel(model, limit = 100) {
	try {
		...
		log(`Error fetching model ${model}: ${err.message}`, 'warn');
		return [];
	}
}
...
async function fetchFromBuilder() {
	...
	try {
		...
		const processedDocumentUrls = new Set();

		for (const model of models) {
			try {
				// Use fetchModel instead of fetchEntries
				...
				for (const entry of entries) {
					...
					// Count chunks by type and track document URLs
					for (const chunk of chunks) {
						if (chunk.sourceType === 'attachment') {
							totalDocumentChunks++;
							// Track unique documents that were successfully processed
							if (chunk.attachmentUrl) {
								processedDocumentUrls.add(chunk.attachmentUrl);
							}
						} else if (chunk.sourceType === 'post') {
							totalArticleChunks++;
						}
					}
				}
		...
		totalDocumentsDownloaded = processedDocumentUrls.size;

		// Summary logging
		log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━', 'info');
		log('📊 BUILDER.IO CONTENT INGESTION SUMMARY', 'success');
		log(`  📄 Articles fetched: ${totalArticles}`, 'info');
		log(`  📋 Article chunks created: ${totalArticleChunks}`, 'info');
		log(
			`  📎 Documents referenced in articles: ${totalDocumentsFound > 0 ? totalDocumentsFound : '(from attachments)'}`,
			'info'
		);
		log(
			`  ✅ Documents successfully downloaded & extracted: ${totalDocumentsDownloaded}`,
			'success'
		);
		log(`  📄 Document chunks created: ${totalDocumentChunks}`, 'success');
		log(`  ═══════════════════════════════════════════════════`, 'info');
		log(
			`  🎯 TOTAL chunks added from Builder.io: ${totalArticleChunks + totalDocumentChunks}`,
			'success'
		);
		log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━', 'info');
		...
		log(`Failed to fetch Builder.io content: ${err.message}`, 'warn');
		return [];
	}
}
...
async function downloadAndExtractDocument(url) {
	if (url.includes('/image/'))
		return { text: '', fileName: fileNameFromUrl(url), fileType: 'image' };
...
async function processBuilderEntry(entry, model) {
	...
	log(`  📎 Found ${docRefs.length} document reference(s) in article "${title}"`);
	...
	if (successfulDownloads > 0) {
		log(
			`  ✅ Successfully extracted ${successfulDownloads} document(s) → ${addedDocChunks} chunk(s)`
		);
	}
...
function chunkText(text, size = 900, overlap = 250) {
	...
	for (const para of paragraphs) {
		...
		if (currentChunk.length > 0 && currentChunk.length + trimmedPara.length + 2 > size) {
...
async function processFile(filePath) {
	...
	const title =
		frontmatterMetadata.title || pathMetadata.filename.replace(/\.[^/.]+$/, '').replace(/_/g, ' ');
	const author = frontmatterMetadata.author || 'Archives Familiales';
	const year = frontmatterMetadata.year || pathMetadata.year;
	const category = frontmatterMetadata.category || pathMetadata.category || 'General';
...
function validateChunks(chunks) {
	...
	const invalid = chunks.filter((c) => !required.every((field) => field in c && c[field]));
...
async function main() {
	...
	// Detailed breakdown
	const localDocChunks = localChunks.length;
	const builderArticleChunks = builderChunks.filter((c) => c.sourceType === 'post').length;
	const builderDocumentChunks = builderChunks.filter((c) => c.sourceType === 'attachment').length;
	...
	log('════════════════════════════════════════════════════════════════');
	log('');
	log('📊 CONTENT BREAKDOWN:', 'info');
	log(`  Local documents:           ${localDocChunks} chunks`, 'info');
	log('');
	log(`  Builder.io articles:       ${builderArticleChunks} chunks`, 'success');
	log(
		`  Builder.io attachments:    ${builderDocumentChunks} chunks (from linked documents)`,
		'success'
	);
	log(`  Builder.io total:          ${builderChunks.length} chunks`, 'info');
	log('');
	log(`  Duplicates removed:        ${allChunks.length - deduped.length} chunks`, 'info');
	log(`  ════════════════════════════════════════════════════════════`, 'info');
	log(`  📁 FINAL UNIQUE CHUNKS:    ${deduped.length}`, 'success');
	log('');
	log('Next steps:', 'info');
