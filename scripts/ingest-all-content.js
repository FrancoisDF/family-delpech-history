...
async function fetchFromBuilder() {
  ...
  try {
    ...
    log(`  📎 Documents referenced in articles: ${totalDocumentsFound > 0 ? totalDocumentsFound : '(from attachments)'}`, 'info');
    log(`  ✅ Documents successfully downloaded & extracted: ${totalDocumentsDownloaded}`, 'success');
    log(`  📄 Document chunks created: ${totalDocumentChunks}`, 'success');
    log(`  ═══════════════════════════════════════════════════`, 'info');
    log(`  🎯 TOTAL chunks added from Builder.io: ${totalArticleChunks + totalDocumentChunks}`, 'success');
    log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━', 'info');
...
async function downloadAndExtractDocument(url) {
  if (url.includes("/image/")) return { text: '', fileName: fileNameFromUrl(url), fileType: 'image' };
...
async function processBuilderEntry(entry, model) {
  ...
  if (successfulDownloads > 0) {
    log(`  ✅ Successfully extracted ${successfulDownloads} document(s) → ${addedDocChunks} chunk(s)`);
  }
...
async function main() {
  ...
  log(`  Builder.io attachments:    ${builderDocumentChunks} chunks (from linked documents)`, 'success');
